...
const JA_VOCAB: Array<{
	...
	},
	{
		native: '兄',
		reading: 'ani',
		translation: 'Hermano mayor (propio)',
		theme: 'people',
		grammar: 'noun'
	},
	{
	...
	{ native: '見る', reading: 'miru', translation: 'Ver / mirar', theme: 'daily', grammar: 'verb' },
	{
		native: '聞く',
		reading: 'kiku',
		translation: 'Escuchar / preguntar',
		theme: 'daily',
		grammar: 'verb'
	},
	...
	},
	{
		native: 'いる',
		reading: 'iru',
		translation: 'Estar / haber (personas/animales)',
		theme: 'daily',
		grammar: 'verb'
	},
	// Adjetivos
...
const KO_VOCAB: Array<{
	...
	{
		native: '죄송합니다',
		reading: 'joesonghamnida',
		translation: 'Lo siento (formal)',
		theme: 'greetings',
		grammar: 'expression'
	},
	{
		native: '처음 뵙겠습니다',
		reading: 'cheoeum boepgesseumnida',
		translation: 'Encantado de conocerle',
		theme: 'greetings',
		grammar: 'expression'
	},
	{
		native: '잘 부탁드립니다',
		reading: 'jal butakdeurimnida',
		translation: 'Me pongo en sus manos',
		theme: 'greetings',
		grammar: 'expression'
	},
	// Números
	{
		native: '일',
		reading: 'il',
		translation: 'Uno (1) — sino-coreano',
		theme: 'numbers',
		grammar: 'noun'
	},
	{
		native: '이',
		reading: 'i',
		translation: 'Dos (2) — sino-coreano',
		theme: 'numbers',
		grammar: 'noun'
	},
	{
		native: '삼',
		reading: 'sam',
		translation: 'Tres (3) — sino-coreano',
		theme: 'numbers',
		grammar: 'noun'
	},
	{
		native: '사',
		reading: 'sa',
		translation: 'Cuatro (4) — sino-coreano',
		theme: 'numbers',
		grammar: 'noun'
	},
	{
		native: '오',
		reading: 'o',
		translation: 'Cinco (5) — sino-coreano',
		theme: 'numbers',
		grammar: 'noun'
	},
	{
		native: '하나',
		reading: 'hana',
		translation: 'Uno (1) — coreano nativo',
		theme: 'numbers',
		grammar: 'noun'
	},
	{
		native: '둘',
		reading: 'dul',
		translation: 'Dos (2) — coreano nativo',
		theme: 'numbers',
		grammar: 'noun'
	},
	{
		native: '셋',
		reading: 'set',
		translation: 'Tres (3) — coreano nativo',
		theme: 'numbers',
		grammar: 'noun'
	},
	// Personas
...
async function seed() {
	...
	const jaId =
		ja?.id ?? (await db.query.languages.findFirst({ where: (l, { eq }) => eq(l.code, 'ja') }))!.id;
	const koId =
		ko?.id ?? (await db.query.languages.findFirst({ where: (l, { eq }) => eq(l.code, 'ko') }))!.id;
	...
	const [hiraganaDeck] = await db
		...
		.values({
			languageId: jaId,
			name: 'Hiragana',
			description: 'Los 46 hiragana básicos',
			isSystem: true
		})
		.onConflictDoNothing()
		.returning();
	...
	if (hiraganaDeck) {
		await db.insert(cards).values(
			HIRAGANA.map(([native, reading]) => ({
				deckId: hiraganaDeck.id,
				native,
				reading,
				translation: `Romaji: ${reading}`
			}))
		);
	}
	...
	const [katakanaDeck] = await db
		...
		.values({
			languageId: jaId,
			name: 'Katakana',
			description: 'Los 46 katakana básicos',
			isSystem: true
		})
		.onConflictDoNothing()
		.returning();
	...
	const [jlptDeck] = await db
		...
		.values({
			languageId: jaId,
			name: 'Vocabulario JLPT N5',
			description: 'Vocabulario esencial nivel N5',
			isSystem: true
		})
		.onConflictDoNothing()
		.returning();
	...
	if (jlptDeck) {
		...
		await db.insert(deckTags).values(
			[...new Set(JA_VOCAB.map((w) => w.theme))].map((tag) => ({
				deckId: jlptDeck.id,
				tag,
				tagType: 'theme' as const
			}))
		);
		await db.insert(deckTags).values(
			[...new Set(JA_VOCAB.map((w) => w.grammar))].map((tag) => ({
				deckId: jlptDeck.id,
				tag,
				tagType: 'grammar' as const
			}))
		);
	}
	...
	const [consonantsDeck] = await db
		...
		.values({
			languageId: koId,
			name: 'Consonantes (자음)',
			description: '19 consonantes del hangul',
			isSystem: true
		})
		.onConflictDoNothing()
		.returning();
	...
	if (consonantsDeck) {
		await db.insert(cards).values(
			KO_CONSONANTS.map(([native, reading]) => ({
				deckId: consonantsDeck.id,
				native,
				reading,
				translation: `Romanización: ${reading}`
			}))
		);
	}
	...
	const [vowelsDeck] = await db
		...
		.values({
			languageId: koId,
			name: 'Vocales (모음)',
			description: '21 vocales del hangul',
			isSystem: true
		})
		.onConflictDoNothing()
		.returning();
	...
	if (vowelsDeck) {
		await db.insert(cards).values(
			KO_VOWELS.map(([native, reading]) => ({
				deckId: vowelsDeck.id,
				native,
				reading,
				translation: `Romanización: ${reading}`
			}))
	...
	const [koVocabDeck] = await db
		...
		.values({
			...
			description: 'Vocabulario esencial coreano',
			isSystem: true
		})
		.onConflictDoNothing()
		.returning();
	...
	if (koVocabDeck) {
		...
		await db.insert(deckTags).values(
			[...new Set(KO_VOCAB.map((w) => w.theme))].map((tag) => ({
				deckId: koVocabDeck.id,
				tag,
				tagType: 'theme' as const
			}))
