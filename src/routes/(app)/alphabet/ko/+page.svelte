<script lang="ts">
	...
	// Consonants: 19, Vowels: 21 — always load all
	const cardLimit = 30;
...
<svelte:head><title>Coreano — Alfabeto</title></svelte:head>

<div class="mx-auto max-w-lg space-y-8">
	<div class="flex items-center gap-4">
		<a href="/alphabet" class="text-sm text-gray-400 hover:text-gray-600">← Alfabeto</a>
		<div>
			<h1 class="text-2xl font-bold text-gray-900">Coreano — Jamo</h1>
			<p class="mt-0.5 text-sm text-gray-500">Practica consonantes y vocales del hangul.</p>
		</div>
	</div>

	<!-- Script selector -->
	<div>
		<p class="mb-3 text-sm font-medium text-gray-700">Elige el conjunto:</p>
		{#if data.decks.length === 0}
			<p class="text-sm text-amber-600">
				No se encontraron mazos de jamo. Ejecuta <code class="rounded bg-gray-100 px-1">bun run db:seed</code>.
			</p>
		{:else}
			<div class="grid gap-3 sm:grid-cols-2">
				{#each data.decks as deck}
					...
					>
						<p class="text-2xl font-medium text-gray-900">{PREVIEWS[deck.name] ?? '한글'}</p>
	...
	<div>
		...
		<div class="flex gap-3">
			...
			>
				Jamo → Lectura
				<span class="mt-1 block text-lg">ㄱ → g/k</span>
			</button>
			<button
				type="button"
			...
			>
				Lectura → Jamo
				<span class="mt-1 block text-lg">g/k → ㄱ</span>
			</button>
		</div>
	</div>
	...
	<!-- Timer -->
	<div>
		<label for="timer-ko" class="mb-2 block text-sm font-medium text-gray-700">
			Tiempo por tarjeta
		</label>
		<select
			id="timer-ko"
